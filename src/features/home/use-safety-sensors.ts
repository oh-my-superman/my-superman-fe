...
export function useSafetySensors(active: boolean) {
  ...
  useEffect(() => {
    ...
    pressureHistoryRef.current = pressureHistoryRef.current.filter(
      (p) => now - p.time < 2000,
    )

    const oneSecAgo = pressureHistoryRef.current.find(
      (p) => now - p.time >= 1000,
    )
    const pressureDelta = oneSecAgo
      ? Math.abs(data.pressure - oneSecAgo.val)
      : 0
    ...
    const hasActivity =
      motionScore > 15 || noiseScore > 20 || rotationScore > 15
    ...
    if (finalScore !== data.dangerScore) {
      ...
      if (finalScore > 90) {
        triggerAlert(
          `[긴급] 매우 위험한 상황이 감지되었습니다! (위험도: ${finalScore}%)`,
        )
      }
    }
  }, [
    ...
    async function startSensors() {
        ...
        try {
          await Promise.all([
            navigator.permissions
              ...
              .catch(() => ({ state: 'granted' })),
            navigator.permissions
              .query({ name: 'gyroscope' as any })
              .catch(() => ({ state: 'granted' })),
            navigator.permissions
              .query({ name: 'ambient-light-sensor' as any })
              .catch(() => ({ state: 'granted' })),
          ])
        } catch (e) {
          /* ignore */
      ...
      if ('Barometer' in window) {
        try {
          ...
          pressureSensor.addEventListener('reading', () => {
            setData((prev) => ({
              ...prev,
              pressure: Math.round(pressureSensor.pressure),
            }))
          })
      ...
      const handleMotion = (event: DeviceMotionEvent) => {
        ...
        if (acc) {
          ...
          setData((prev) => ({
            ...prev,
            motion: Math.round(magnitude * 10) / 10,
          }))
        }
